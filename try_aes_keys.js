...
const keyStr = "7x!A%D*G-KaPdSgV";
const Key = CryptoJS.enc.Utf8.parse(keyStr);
const iv = CryptoJS.enc.Utf8.parse(keyStr);

try {
    const dec = CryptoJS.AES.decrypt(privateClientEn, Key, {
        keySize: 128 / 8,
        iv: iv,
        mode: CryptoJS.mode.CBC,
        padding: CryptoJS.pad.Pkcs7
    }).toString(CryptoJS.enc.Utf8);
    console.log("Decrypted privateClientEn:", dec.substring(0, 100));
} catch(e) {
    console.log("Failed to decrypt privateClientEn");
}

try {
    const dec2 = CryptoJS.AES.decrypt(bigPriEn, Key, {
        keySize: 128 / 8,
        iv: iv,
        mode: CryptoJS.mode.CBC,
        padding: CryptoJS.pad.Pkcs7
    }).toString(CryptoJS.enc.Utf8);
    console.log("Decrypted bigPriEn:", dec2.substring(0, 100));
} catch(e) {
    console.log("Failed to decrypt bigPriEn");
}
