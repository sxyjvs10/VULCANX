const crypto = require('crypto');

function decrypt(text, keyBase64) {
    const keyStr = Buffer.from(keyBase64, 'base64').toString('utf8');
    const key = Buffer.from(keyStr, 'utf8');
    const iv = Buffer.from(keyStr, 'utf8');
    
    const decipher = crypto.createDecipheriv('aes-128-cbc', key, iv);
    let decrypted = decipher.update(text, 'base64', 'utf8');
    decrypted += decipher.final('utf8');
    return decrypted;
}

const k = "N3glRCpHLUthUGRTZ1YkQA==";
const k1 = "N3ghQSVEKkctS2FQZFNnVg==";

const privateClientEn = "";

const bigPriEn = "";

console.log("--- privateClientEn (k) ---");
try { console.log(decrypt(privateClientEn, k)); } catch(e) { console.log("Failed"); }
console.log("--- privateClientEn (k1) ---");
try { console.log(decrypt(privateClientEn, k1)); } catch(e) { console.log("Failed"); }

console.log("--- bigPriEn (k) ---");
try { console.log(decrypt(bigPriEn, k)); } catch(e) { console.log("Failed"); }
console.log("--- bigPriEn (k1) ---");
try { console.log(decrypt(bigPriEn, k1)); } catch(e) { console.log("Failed"); }
